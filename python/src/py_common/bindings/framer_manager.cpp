...
nb::tuple PyFramerManager::PyIterGetFrame()
{
    ...
    catch (py_common::IncompleteException)
    {
    }
    catch (py_common::IncompleteMoreDataException)
    {
    }
    catch (py_common::BufferEmptyException)
    {
    }
...
void init_common_framer_manager(nb::module_& m)
{
    nb::class_<PyFramerManager>(m, "FramerManager")
        ...
        .def_prop_ro("active_framer_name", &PyFramerManager::GetActiveFramerName, "The name of the framer that successfully framed the last message")
        .def(
            "write",
        ...
        .def(
            "__iter__", [](nb::handle_t<PyFramerManager> self) { return self; }, nb::sig("def __iter__(self) -> Iterator[tuple[bytes, MetaData]]"),
            R"doc(
