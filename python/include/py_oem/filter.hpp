...
class PyFilter : public Filter
{
  public:
    // -------------------------------------------------------------------------------------------------------
    void PySetIncludeLowerTimeBound(py_oem::PyGpsTime* value)
    {
        if (value == nullptr) { ClearLowerTimeBound(); }
        else { SetIncludeLowerTimeBound(value->week, value->milliseconds / 1000.0); }
    }
    void PySetIncludeUpperTimeBound(py_oem::PyGpsTime* value)
    {
    ...
    {
        if (bMyFilterLowerTime) { return nb::cast(py_oem::PyGpsTime(uiMyLowerWeek, uiMyLowerMSec)); }
        else { return nb::none(); }
    }
    nb::object PyGetIncludeUpperTimeBound()
    {
        if (bMyFilterUpperTime) { return nb::cast(py_oem::PyGpsTime(uiMyUpperWeek, uiMyUpperMSec)); }
        else { return nb::none(); }
    }
    ...
    // -------------------------------------------------------------------------------------------------------
    std::vector<std::tuple<uint32_t, INPUT_FORMAT, uint8_t>> PyGetMessageIds() { return vMyMessageIdFilters; }
    bool PyMessageIdFilterInverted() { return bMyInvertMessageIdFilter; }

    // -------------------------------------------------------------------------------------------------------
    std::vector<std::tuple<std::string, INPUT_FORMAT, uint8_t>> PyGetMessageNames() { return vMyMessageNameFilters; }
