...
class PyMessageDatabaseCore : public MessageDatabase
{
  public:
    ...
    //-----------------------------------------------------------------------
    void RemoveEnumType(const std::string& enum_id);
    //-----------------------------------------------------------------------
...
class PyMessageDatabase
{
  public:
    ...
    void PyAppendMessages(const std::vector<MessageDefinition::ConstPtr>& vMessageDefinitions_)
    {
        ...
        pclMessageDb->AppendMessageTypes(vMessageDefinitions_);
    }

    void PyAppendEnumerations(const std::vector<EnumDefinition::ConstPtr>& vEnumDefinitions_)
    {
        pclMessageDb->AppendEnumerations(vEnumDefinitions_);
        pclMessageDb->AppendEnumTypes(vEnumDefinitions_);
    }

    void PyRemoveMessage(const uint32_t iMsgId_)
    {
        pclMessageDb->RemoveMessage(iMsgId_);
        pclMessageDb->RemoveMessageType(iMsgId_);
    }

    void PyRemoveEnumeration(std::string_view strEnumeration_)
    {
        pclMessageDb->RemoveEnumeration(strEnumeration_);
        pclMessageDb->RemoveEnumeration(strEnumeration_);
    }
    ...
    [[nodiscard]] const std::unordered_map<std::string, PyMessageType>& GetMessagesByNameDict() const
    {
  ...
  private:
    ...
    void ThrowRXConfigConstructionError() const { throw FailureException("RXConfig definition in database cannot be handled."); }
