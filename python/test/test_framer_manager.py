...
class Helper:
    def __init__(self, test_resources_path):
        ...
        self.framer_manager = ne.FramerManager(["OEM"])
        self.framer_manager.report_unknown_bytes = True
        pass

    def test_framer_manager_errors(self, expected_error, buffer_size=ne.MAX_MESSAGE_LENGTH):
        pass
    ...
    def write_bytes_to_framer_manager(self, data):
        assert self.framer_manager.write(data) == len(data)

    def write_file_to_framer(self, filename):
        data = self.get_file_contents(filename)
        self.write_bytes_to_framer_manager(data)
...
def test_ascii_incomplete(helper):
    ...
    helper.write_bytes_to_framer_manager(data)
    helper.test_framer_manager_errors(ne.BufferEmptyException)
...
def test_ascii_segmented(helper):
    ...
    helper.write_bytes_to_framer_manager(data[bytes_written:][:135])
    bytes_written += 135
    helper.test_framer_manager_errors(ne.BufferEmptyException)

    helper.write_bytes_to_framer_manager(data[bytes_written:][:1])
    bytes_written += 1
    helper.test_framer_manager_errors(ne.BufferEmptyException)
...
def test_abbrev_ascii_segmented(helper):
    ...
    bytes_written = 0
    helper.write_bytes_to_framer_manager(data[bytes_written:][:1])  # Sync Byte
    bytes_written += 1
    helper.test_framer_manager_errors(ne.BufferEmptyException)

    helper.write_bytes_to_framer_manager(data[bytes_written:][:69])  # Header with no CRLF
    bytes_written += 69
    helper.test_framer_manager_errors(ne.BufferEmptyException)

    helper.write_bytes_to_framer_manager(data[bytes_written:][:1])  # CR
    bytes_written += 1
    helper.test_framer_manager_errors(ne.BufferEmptyException)

    helper.write_bytes_to_framer_manager(data[bytes_written:][:1])  # LF
    bytes_written += 1
    helper.test_framer_manager_errors(ne.BufferEmptyException)

    helper.write_bytes_to_framer_manager(data[bytes_written:][:89])  # Body
    bytes_written += 89
    helper.test_framer_manager_errors(ne.BufferEmptyException)
...
def test_binary_segmented(helper):
    ...
    helper.write_bytes_to_framer_manager(data[bytes_written:][:72])
    bytes_written += 72
    expected_meta_data.length = bytes_written
    helper.test_framer_manager_errors(ne.BufferEmptyException)
...
def test_short_ascii_segmented(helper):
    ...
    helper.write_bytes_to_framer_manager(data[bytes_written:][:82])
    bytes_written += 82
    helper.test_framer_manager_errors(ne.BufferEmptyException)
