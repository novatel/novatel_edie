...
# Patch ct.c_bool be 4 bytes in size
_c_bool = ct.c_uint32

DLL_MSG_FRAME_SIZE = DECODERS_DLL.get_frame_size()

DECODERS_DLL.version.restype = ct.c_char_p
DECODERS_DLL.version.argtypes = None
DECODERS_DLL.pretty_version.restype = ct.c_char_p
DECODERS_DLL.pretty_version.argtypes = None

DECODERS_DLL.framer_init.restype = ct.c_void_p
DECODERS_DLL.framer_init.argtypes = [ct.c_void_p, ct.c_void_p]
DECODERS_DLL.framer_del.restype = None
DECODERS_DLL.framer_del.argtypes = [ct.c_void_p]
DECODERS_DLL.framer_read.restype = None
DECODERS_DLL.framer_read.argtypes = [ct.c_void_p, ct.c_void_p, ct.c_void_p]

DECODERS_DLL.decoder_init.restype = ct.c_void_p
DECODERS_DLL.decoder_init.argtypes = [ct.c_void_p, ct.c_void_p, ct.c_void_p]
DECODERS_DLL.decoder_del.restype = None
DECODERS_DLL.decoder_del.argtypes = [ct.c_void_p]
DECODERS_DLL.decoder_read.restype = _c_bool
DECODERS_DLL.decoder_read.argtypes = [ct.c_void_p, ct.c_void_p, ct.c_void_p, ct.c_void_p, ct.c_void_p]
DECODERS_DLL.decoder_copy_header.restype = ct.c_void_p
DECODERS_DLL.decoder_copy_header.argtypes = [ct.c_void_p, ct.c_void_p]
DECODERS_DLL.decoder_copy_body.restype = ct.c_void_p
DECODERS_DLL.decoder_copy_body.argtypes = [ct.c_void_p, ct.c_void_p]
...
class DecoderMessageHeader(ct.Structure):
    ...
    _fields_ = [
        ...
        ("week", ct.c_ulong),
        ("time", ct.c_ulong),
        ("message_format", ct.c_ulong),
        ("time_status", ct.c_ulong),
        ("receiver_status", ct.c_ulong),
        ("sw_version", ct.c_ulong),
        ("is_response", ct.c_ulong),
    ]
...
class FramerMessageHeader(ct.Structure):
    ...
    _fields_ = [
        ...
        ("sw_version", ct.c_ulong),
        ("is_response", ct.c_ulong),
        ("log_data", (ct.c_ubyte * DecoderMessageHeader.FRAME_DATA_SIZE)),
    ]
    ...
    def __str__(self):
        return ",".join(str(x) for x in [
            ...
            self.receiver_status,
            self.sw_version,
            ct.string_at(self.log_data),
        ])

    def get_data_str(self):
        return ct.string_at(self.log_data)
...
class Framer:
    ...
    def next(self):
        ...
        """
        DECODERS_DLL.framer_read(self._framer, ct.byref(self._current_status), ct.byref(self._current_log))
...
class Decoder:
    ...
    def __init__(self, input_stream, json_database=_util.JSON_DB_PATH, msg_filter=None):
        ...
        self._msg_data = ct.create_string_buffer(DLL_MSG_FRAME_SIZE)
        self._msg_json_data = ct.create_string_buffer(DLL_MSG_FRAME_SIZE * 2)
        self._ascii_current_body = str

        self._current_log = Log()
        self._current_log.header = self._current_header
        self._unknown_binary_id = ct.c_uint32(-1).value
    ...
    def read_decoded_message(self, unknown_data=True):
        ...
        while True:
            read_status = DECODERS_DLL.decoder_read(self._Decoder,
                                                    ct.byref(self._status),
                                                    ct.byref(self._current_header),
                                                    self._msg_data,
                                                    self._msg_json_data)
