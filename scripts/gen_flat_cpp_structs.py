...
def gen_flat_structs(msg_database: dict, out_file: str = 'novatel_message_definitions.hpp', all_logs: bool = False):
    ...
    for msg in msg_database['logs']:
        if not all_logs and msg['name'] not in WEBSITE_LOGS:
            continue
        gen_field_defs(msg['fields'], msg['name'])

    with open(out_file, 'w') as fp:
        fp.write('#include "decoders\\common\\api\\common.hpp\n#include "decoders\\common\\api\\env.hpp"\n\n#pragma pack(1)\n\n');
    ...
    for field in fields:
        ...
        if field['type'] == 'SIMPLE':
            if field['dataType']['name'] == 'ENUM':
                ...
                msg_fields.append(f'LONG {field["name"]}')
            else:
                msg_fields.append(f'{field["dataType"]["name"]} {field["name"]}')
        elif field['type'] == 'STRING':
            msg_fields.append(f'UCHAR {field["name"]}[{field["arrayLength"]}]')
        elif field['type'] == 'FIXED_LENGTH_ARRAY':
            msg_fields.append(f'{field["dataType"]["name"]} {field["name"]}[{field["arrayLength"]}]')
        elif field['type'] == 'VARIABLE_LENGTH_ARRAY':
            msg_fields.append(f'ULONG {field["name"]}_arraylength')
            msg_fields.append(f'{field["dataType"]["name"]} {field["name"]}[{field["arrayLength"]}]')
...
def parse_args():
    ...
    p.add_argument('-ts', '--tab_space', action='store_true', help='Use 4 spaces instead of a tab character',
                   default=False)
    p.add_argument('-al', '--all-logs', action='store_true', default=False, help='Default is website logs only. Use this switch to generate structs for all logs')
    return p.parse_args()
    

if __name__ == '__main__':
    warnings.warn('This script is incompatible with the json file in the database folder. A compatible json file will be released in the future.')
    print()
